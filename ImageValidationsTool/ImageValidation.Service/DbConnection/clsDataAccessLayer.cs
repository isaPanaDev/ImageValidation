...
public static class clsDataAccessLayer
{
    ...
    public class clsServiceMethods
    {
        ...
        public static int SaveComputerInformationFromXml(Computer objComputer, DataTable dtDriver, DataTable dtApplication, DataTable dtHotfix)
        {
            ...
            cmdProc.Parameters.AddWithValue("@SerialNumber", objComputer.SerialNumber);
            cmdProc.Parameters.AddWithValue("@SystemDrive", objComputer.SystemDrive);
        ...
        #region INSERT DATA INTO Product Table (For Search)
        public static bool InsertProduct(string Product)
        {
            bool Status = false;
            objdt = new DataTable();
            ...
            else
            {
                try
                {
                    sql = "INSERT INTO Product(ProductName) Values('" + Product + "')";
                    if (ADOC.GetObject.ExecuteDML(sql))
                    {
                        Status = true;
                    }
